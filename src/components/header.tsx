...
export default function Header() {
  ...
  return (
    ...
    >
        ...
        <nav className="hidden md:flex items-center space-x-8">
          {navItems.map((item) =>
            item.href.startsWith("/#") ? (
              <a key={item.name} href={item.href} className={cn("nav-link", isActive(item.href) && "active")}>
                {item.name}
              </a>
            ) : (
              <Link key={item.name} to={item.href} className={cn("nav-link", isActive(item.href) && "active")}>
                {item.name}
              </Link>
            ),
          )}
      ...
      {/* Search Overlay */}
      {isSearchOpen && (
        <div className="absolute top-full left-0 right-0 bg-background shadow-lg p-4 border-t border-border animate-in fade-in-0 slide-in-from-top-5">
          <div className="container-custom">
            <input
              type="text"
              placeholder="Keresés..."
              className="w-full p-3 bg-secondary/50 rounded-lg"
              aria-label="Keresés"
              autoFocus
            />
          </div>
      ...
      {isMenuOpen && (
        <div className="absolute top-full left-0 right-0 bg-background shadow-lg border-t border-border md:hidden animate-in fade-in-0 slide-in-from-top-5">
            ...
            {navItems.map((item) =>
              item.href.startsWith("/#") ? (
                <a
                  ...
                  href={item.href}
                  className={cn(
                    "py-2 px-4 hover:bg-secondary rounded-lg",
                    isActive(item.href) && "bg-secondary text-primary font-semibold",
                  )}
